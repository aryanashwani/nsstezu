...
function getData(){
    ...
    fetch("https://script.google.com/macros/s/AKfycbyHr01wShawJBOIqgbvVu0_flYrHp4g9odQ7oYnyEvHawZm1gCoy-xcNz5P8wwyGMLEoQ/exec",
    {method:'GET'} ,
    (response)=>{
            ...
            for(year in data){
                ...
                for(branch in data[year]){
                    ...
                    for(row in data[year][branch]){
                        ...
                        `<tr>
                        <td>${row}</td>
                        <td>${data[year][branch][row].roll}</td>
    ...
    if(branch=='all'){
        for(branch_ in data[year]){
            for(row in data[year][branch_]){
                ...
                <td>${row}</td>
                <td>${data[year][branch_][row].roll}</td>
